...
const Heroes = () => {
    ...
    return (
        <div className={s.main}>
            <div>
                <button style={queue!=='pro'?({color:'#3f3f3f'}):({color:'white'})} className={s.button} onClick={() => setQueue('pro')}>Pro</button>
                <button style={queue=='pro'?({color:'#3f3f3f'}):({color:'white'})} className={s.button} onClick={() => setQueue('turbo')}>Turbo</button>
            </div>
