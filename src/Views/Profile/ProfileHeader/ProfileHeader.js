    ...
    return (
        <table className={s.main}>
            <div className={s.avatar_n_info}>
                <img className={s.avatar} src={player.profile.profile.avatarfull}/>
                ...
                <div className={s.head_info}>
                    ...
                    <div className={s.wl}> {((player.wl.win / (player.wl.win + player.wl.lose)) * 100).toFixed(1)}%</div>
                </div>
            </div>

            <div className={s.last_visit}>
                <div>last visit: {player.profile.profile.last_login ? (moment(player.profile.profile.last_login).fromNow()) : ('long time ago')}</div>
                <div>last match: {player.recentMatches[0] ? (moment(getUnixTime(player.recentMatches[0].start_time)).fromNow()) : ('long time ago')}</div>
            </div>
            <div>
                <Rank player={player}/>
            </div>
